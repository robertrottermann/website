...
class WebsiteEvent(website_event):
    ...
    def cart_update(self,**post):
        ...
        for key, value in post.items():
            ...
            ticket_id = (ticket_words[0] == 'ticket' and int(ticket_words[1]) or None)
            ...
            if not ticket.price:
                ...
                post['free_tickets'] = (str(int(post['free_tickets']) + qty))
            else:
                has_paid_tickets = True
                # Add to shopping cart the rest of the items
                order = request.website.sale_get_order(force_create=1)
                order.with_context(event_ticket_id=ticket.id)._cart_update(product_id=ticket.product_id.id, add_qty=qty)
